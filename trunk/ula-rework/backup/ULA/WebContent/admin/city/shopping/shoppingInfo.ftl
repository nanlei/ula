...
<form name="articleForm" action="updateShopping.action?articleId=${shoppingInfo.ID}" method="post" onsubmit="return validateForm(this)">
<table class="default" width="100%">
	...
	<tr>
		<td>标题</td><td colspan="3"><input type="text" name="title" emptyInfo="请填写文章标题" value="${shoppingInfo.TITLE}" size="80"></td>
	</tr>
	<#--
		<td>封面图片</td>
		<td>
			<input type="text" name="coverLink" emptyInfo="请先选择封面图片，然后填写图片的链接地址" title="图片的链接地址应该从/images开始" value="${shoppingInfo.COVERLINK}">&nbsp;
	...
	<tr>
		<td>参考价格</td><td><input type="text" name="price" value="${shoppingInfo.PRICE}" numberInfo="请填写价格，并且是数字格式"></td>
		<td></td><td></td>
	</tr>-->
	<tr>
		<td colspan="4"><textarea name="content" id="content">${shoppingInfo.CONTENT}</textarea><@p.ckeditor id="content"/></td>
	</tr>
	<tr>
