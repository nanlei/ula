...
/**
 * About us -> business introduction
 * 
 * @author Harry
 * 
 */
public class BusinessAction extends FrameworkAction {
	/**
	...
	protected int addToDB(String title, String content) {
		try {
			return super.getServiceManager().getAboutService().addBizInfo(
					title, content);
		} catch (Exception e) {
	...
	protected int updateToDB(String title2, String content2) {
		try {
			return this.getServiceManager().getAboutService().updateBizInfo(
					title2, content2);
		} catch (Exception e) {
