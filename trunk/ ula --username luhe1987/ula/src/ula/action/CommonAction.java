...
public class CommonAction extends BaseAction {
	...
	/**
	 * 公有的view方法，用来存放页面的公用信息
	 *如友情链接、本月推荐标题
	 * @return
	 */
	public String view(){
		PagingList linksFromSession = (PagingList)super.getHttpSessionAsMap().get(CommonConstants.LINK);
...
//			友情链接
			PagingList link = super.getServiceManager().getLinkService().getAllInfo();
...
//检查Session中是否已过期，如果可以去到信息，则session没有过期
			Map recommendation = (Map) super.getHttpSessionAsMap().get(CommonConstants.RECOMMENDATION);
			if(recommendation==null||recommendation.isEmpty()){
//				获得本月推荐的所有内容
			    recommendation = super.getServiceManager().getProgramService().getInfo(CommonConstants.ID_PROGRAM_MONTH_RECOMMENDATION);
//				获取title
				recomTitle = (String) recommendation.get("title");
				super.debug("推荐标题：" + recomTitle);
//				放入Session
				super.getHttpSessionAsMap().put(CommonConstants.RECOMMENDATION, recomTitle);
			}
			else{
