...
public class CompanyAction extends FrameworkAction {
	...
	public String edit() {
		...
		try {
			super.setInfoMap(super.getServiceManager().getAboutService()
					.getCompanyInfo());
		} catch (Exception e) {
			log.error(ExceptionUtils.getStackTrace(e));
			return ERROR;
	...
	protected int addToDB(String title, String content) {
		try {
			return super.getServiceManager().getAboutService().addCompanyInfo(
					title, content);
		} catch (Exception e) {
	...
	protected int updateToDB(String title2, String content2) {
		try {
			return this.getServiceManager().getAboutService()
					.updateCompanyInfo(title2, content2);
		} catch (Exception e) {
