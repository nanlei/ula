...
public class PagingList {
	...
	private int pageSize = CommonConstants.DEFAULT_PAGE_SIZE; // 每页记录数
	...
	private void execute(JdbcTemplate jdbcTemplate, Object... params) {
		...
		this.list = jdbcTemplate.queryForList(getPagingSql(srcSql,
				CommonConstants.NORMAL_MARK, CommonConstants.NORMAL_MARK), tmp);
	}
	...
	private void execute(NamedParameterJdbcTemplate namedParameterJdbcTemplate,
		...
		if (DBTYPE.equals("MYSQL")) {// MySQL和Oracle参数不同
			namedParams.put(CommonConstants.START_MARK, new Integer(
					getStartIndex() - 1));
			namedParams.put(CommonConstants.END_MARK, new Integer(pageSize));
		} else {
			namedParams.put(CommonConstants.START_MARK, getStartIndexInteger());
			namedParams.put(CommonConstants.END_MARK, getEndIndexInteger());
		}
		this.list = namedParameterJdbcTemplate.queryForList(getPagingSql(
				srcSql, CommonConstants.START_MARK, CommonConstants.END_MARK),
				namedParams);
	}
